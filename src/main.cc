...
// RTOS Tasks
uint32_t g_rtos_stat_timer_cnt = 0;
void RTOS_AppConfigureTimerForRuntimeStats(void) {
    ns_lp_printf("RTOS_AppConfigureTimerForRuntimeStats\n");
}
uint32_t RTOS_AppGetRuntimeCounterValueFromISR(void) {
    g_rtos_stat_timer_cnt++;
...
void send_slot0_signals() {
    ...
    tio_usb_send_slot_data(0, 0, (uint8_t *)buffer, length * sizeof(int16_t));
}
...
void ProcessTask(void *pvParameters) {
    ...
    while (true) {
        ...
        if (ringbuffer_len(&rbEcgDen) >= ECG_DEN_WINDOW_LEN) {
            val_u32 = ulTaskGetIdleRunTimePercent();
            ns_lp_printf("CPU Idle: %d\n", val_u32);
        ...
        ///////////////////////////////////////////////
            ...
            if (appState.segMode == SegmentationModeDsp) {
                ...
                err = ecg_segmentation_inference(ecgSegInout, ecgSegMask, 0, ECG_SEG_THRESHOLD);
            } else{
                err = 0;
        ...
        ///////////////////////////////////////////////
            ...
            if (appState.arrMode == ArrhythmiaModeDsp) {
                ...
                appMetResults.arrhythmiaLabel = ecg_arrhythmia_inference(ecgMetData, ECG_ARR_THRESHOLD);
            } else {
